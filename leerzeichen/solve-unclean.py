# INCORRECT: flag[dd HIss essverssrochen]
...
with open("chal.lrz", "rb") as f:
        ...
        while i < len(ops):
            ...
            if is_int(op):
                ...
                while i + j < len(ops) and is_int(ops[i + j]):
                    ints.append(ops[i + j])
                    j += 1
                # if j == 4 and ints == [8200, 8193, 8202, 8194]:
                #     # print(i, j)
                #     # for _ in range(3, j):
                #     #     ints.pop()
                #     # j = 3
                ...
                # st[-1], st[-2], st[-3] = st[-3], st[-1], st[-2]
                # st[-3], st[-2], st[-1] = st[-2], st[-1], st[-3]
                # st[0], st[1], st[2] = st[1], st[2], st[0]
            elif op == 0x3000:
    ...
    # res = [102, 108, 97, 103, 91, 96, 117, 100, 64, 73, 116, 115, 32, 32, 101, 115, 118, 101, 114, 112, 115, 114, 111, 99, 104, 101, 110, 93]
    # print("".join(map(chr, map(abs, res))))
